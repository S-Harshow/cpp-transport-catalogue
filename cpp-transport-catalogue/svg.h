...
std::ostream &operator<<(std::ostream &ostream, StrokeLineCap line);

std::ostream &operator<<(std::ostream &ostream, StrokeLineJoin line);
...
constexpr bool operator==(const Rgba &lhs, const Rgba &rhs) {
  return lhs.red == rhs.red && lhs.blue == rhs.blue && lhs.green == rhs.green &&
         std::abs(lhs.opacity - rhs.opacity) < EPSILON;
}
