...
ostream &operator<<(ostream &out, const StrokeLineCap line) {
  switch (line) {
...
ostream &operator<<(ostream &out, const StrokeLineJoin line) {
  switch (line) {
...
istream &operator>>(std::istream &in_, Color &color) {
  string line;
  in_ >> line;
...
void Text::RenderObject(const RenderContext &context) const {
  auto &out = context.out;
  //<text fill="rgba(255,255,255,0.85)" stroke="rgba(255,255,255,0.85)"
  // stroke-width="3" stroke-linecap="round" stroke-linejoin="round" x="99.2283"
  // y="329.5" dx="7" dy="15" font-size="20" font-family="Verdana"
  // font-weight="bold">114</text>
  out << "<text"sv;
  // Выводим атрибуты, унаследованные от PathProps
  RenderAttrs(context.out);
