...
inline std::map<std::string, Type> getMap(const Dict &map,
  ...
  if (const auto iter = map.find(json_var_name);
      iter != map.end() && iter->second.IsMap()) {
    Dict son_map = iter->second.AsMap();
...
std::optional<Requests> ParserRenderSettings::parseSection(const Node &node) {
  ...
  auto width = getValue<double>(settings, io::json::detail::JSON_RENDER_WIDTH);
  auto height =
      getValue<double>(settings, io::json::detail::JSON_RENDER_HEIGHT);
  auto padding =
      getValue<double>(settings, io::json::detail::JSON_RENDER_PADDING);
  result.setSize(width, height, padding);

  auto stop_radius =
      getValue<double>(settings, io::json::detail::JSON_RENDER_STOP_RADIUS);
  auto stop_label_font_size = getValue<int>(
      settings, io::json::detail::JSON_RENDER_STOP_LABEL_FRONT_SIZE);
  auto stop_label_offset = getArray<double, 2>(
      settings, io::json::detail::JSON_RENDER_STOP_LABEL_OFFSET);
  result.setStopExterior(stop_radius, stop_label_font_size, stop_label_offset);

  auto bus_label_font_size = getValue<int>(
      settings, io::json::detail::JSON_RENDER_BUS_LABEL_FONT_SIZE);
  auto bus_label_offset = getArray<double, 2>(
      settings, io::json::detail::JSON_RENDER_BUS_LABEL_OFFSET);
  result.setBusExterior(bus_label_font_size, bus_label_offset);

  auto underlayer_width = getValue<double>(
      settings, io::json::detail::JSON_RENDER_UNDERLAYER_WIDTH);

  svg::Color underlayer_color = svg::NoneColor;
  if (const auto iter =
          settings.find(io::json::detail::JSON_RENDER_UNDERLAYER_COLOR);
      iter != settings.end()) {
  ...
  auto line_width =
      getValue<double>(settings, io::json::detail::JSON_RENDER_LINE_WIDTH);
  result.setLineWidth(line_width);

  std::vector<svg::Color> color_palette{};
  if (const auto iter =
          settings.find(io::json::detail::JSON_RENDER_COLOR_PALETTE);
...
void transport::JsonOutputter::SetResponses(const Responses &responses) {
  Array root_array{};
  transform(
      responses.begin(), responses.end(), back_inserter(root_array),
      [](const Response &response) {
        return std::visit(
            transport::io::json::detail::JsonNodeBuilder{response.requestId},
            response.value);
      });
  if (!root_array.empty()) {
...
void transport::JsonInputter::ParseInput() {
  ...
  for_each(root.begin(), root.end(), [this](const auto &section) {
    ...
    auto parser = io::json::detail::ParserBuilder::CreateParser(section.first);
...
Node JsonNodeBuilder::operator()(const BusStat &info) const {
  Dict info_map{};
  info_map.emplace(detail::JSON_RESPONSE_ID, Node(request_id));
  info_map.emplace(detail::JSON_BUS_CURVATURE,
                   Node(double(info.routelength) / double(info.geolength)));
  info_map.emplace(detail::JSON_BUS_ROUTE_LENGTH,
                   Node(static_cast<int>(info.routelength)));
  info_map.emplace(detail::JSON_BUS_STOP_COUNT,
                   Node(static_cast<int>(info.stops)));
  info_map.emplace(detail::JSON_BUS_UNIQUE_STOP_COUNT,
...
Node JsonNodeBuilder::operator()(const MapStat &info) const {
  Dict info_map{};
  info_map.emplace(detail::JSON_RESPONSE_ID, Node(request_id));
  info_map.emplace(detail::JSON_RESPONSE_MAP, Node(info.map_string));
  return {info_map};
}
...
Node JsonNodeBuilder::operator()(const std::monostate & /*unused*/) const {
  Dict info_map{};
  info_map.emplace(detail::JSON_RESPONSE_ID, Node(request_id));
  info_map.emplace(detail::JSON_ERROR_MESSAGE,
                   Node(std::string(detail::JSON_ERROR_NOT_FOUND)));
  return {info_map};
}
...
std::optional<Requests> Parser::parseSection(const Node &map) {
  ...
  for (const auto &request_node : array) {
    if (!request_node.IsMap()) {
      continue;
    }
    const Dict &request = request_node.AsMap();
    auto request_type =
        getValue<string>(request, io::json::detail::JSON_REQUEST_TYPE);
    if (request_type.empty()) {
      continue;
    }
    if (io::json::detail::JSON_REQUEST_BUS == request_type) {
      if (auto query = parseBusNode(request); query.has_value()) {
        result.emplace_back(query.value());
      }
    }
    if (io::json::detail::JSON_REQUEST_STOP == request_type) {
      if (auto query = parseStopNode(request); query.has_value()) {
        result.emplace_back(query.value());
      }
    }
    if (io::json::detail::JSON_REQUEST_MAP == request_type) {
...
std::optional<Request> Parser::parseBusNode(const Dict &map) {
  auto name = getValue<string>(map, io::json::detail::JSON_NAME);
  if (name.empty()) {
    return nullopt;
  }
  BusQuery result(name);
  result.is_roundtrip =
      getValue<bool>(map, io::json::detail::JSON_IS_ROUNDTRIP);
  result.stops = getVector<string>(map, io::json::detail::JSON_STOPS);
...
std::optional<Request> Parser::parseStopNode(const Dict &map) {
  ...
  result.latitude = getValue<double>(map, io::json::detail::JSON_LATITUDE);
  result.longitude = getValue<double>(map, io::json::detail::JSON_LONGITUDE);
  result.road_distances =
      getMap<int>(map, io::json::detail::JSON_ROAD_DISTANCES);
  return make_optional<Request>({result, 0});
}
...
std::optional<Request> ParserStat::parseBusNode(const Dict &map) {
  if (auto request = Parser::parseBusNode(map); request.has_value()) {
    request->requestId = getValue<int>(map, io::json::detail::JSON_REQUEST_ID);
    return request;
  }
  return nullopt;
}
